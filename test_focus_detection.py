import os
import pandas as pd
from modules.module5_evaluation_engine import _pick_target_table_and_col

DATADIR = "sample_data_extreme"

def _normalize_table_name(filename: str) -> str:
    base = os.path.splitext(filename)[0]
    base = base.replace("confidential_", "")
    base = base.replace("-", "_").replace(" ", "_")
    return base.title()

files = [f for f in os.listdir(DATADIR) if f.endswith('.csv')]
real_tables = {}
for f in files:
    path = os.path.join(DATADIR, f)
    name = _normalize_table_name(f)
    real_tables[name] = pd.read_csv(path)

focus_table, focus_col = _pick_target_table_and_col(real_tables)
print(f"Auto-detected focus table: {focus_table}, target column: {focus_col}")
